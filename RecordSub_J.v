...
(** **** 練習問題: ★★, optional *)
Example typing_example_2 : (* yoshihiro503 *)
  has_type empty
...
(* empty |- (\z:(C->C)->{j:B->B}. (z (\x:C.x)).j)
           ...
           : B->B *)
Proof with eauto.
  apply T_App with (ty_arrow (ty_arrow C C) ty_rcd_j).
  - apply T_Abs.
    + apply wfty_arrow...
      apply wfty_rcons...
    + apply T_Proj with ty_rcd_j...
      apply T_App with (ty_arrow C C)...
      apply T_Var...
      apply wfty_arrow...
      apply wfty_rcons...
  - apply T_Abs...
    apply T_Sub with (ty_rcons k (ty_arrow A A) (ty_rcons j (ty_arrow B B) ty_rnil)).
    + apply T_RCons...
    + apply S_Trans with (ty_rcons j (ty_arrow B B) (ty_rcons k (ty_arrow A A) ty_rnil)).
      * apply S_RcdPerm...
      * apply S_RcdDepth...
Qed.
